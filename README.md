<h1>📜 윤일식 포트폴리오</h1>

</br>
</br>
<h2>👋 Intro</h2>
<p>안녕하세요! "항상 새로운 것을 배우고 도전하는" 윤일식입니다</p>
<p>영상 및 음악 제작의 커리어를 뒤로 하고 게임 서버 프로그래머의 꿈을 이루기 위해 새로운 도전을 하게 되었습니다</p>
<p>프로카데미 게임 서버 아카데미에서 공부 후 그 지식을 기반으로 팀 프로젝트를 진행하고 있습니다.</p>
<p>팀 프로젝트 파일은 공개되지 않아 첨부된 pdf문서의 내용을 참조해주시면 감사하겠습니다.</p>
<p>게임 서버 뿐 아니라 실생활에서 사용할 수 있는 python 자동화 프로그램도 개발하고 있습니다.</p>


</br>
<p>아래 내용은 확장 가능한 고성능 게임 서버를 만들기 위해 공부했던 내용입니다.</p>

</br>
</br>
</br>
<h2>📝Projects</h2>
<p>게임 서버를 공부하면서 만들었던 개인 프로젝트입니다.</p>
<p>주어진 코드는 없었고 전부 경험해나가며 구현했습니다.</p>
<p>모든 프로젝트는 단순 구현에서 그치지 않고 더 많은 접속자를 처리하기 위해 최적화 했습니다.</p>
<p>프로젝트 보러 가기 링크를 클릭하시면 상세한 프로젝트에 대한 설명이 있습니다</p>
</br>
</br>
</br>
<h2>1.  메인 서버 분산 설계</h2>
<h3>네트워크 라이브러리 - 로그인 - Redis - 채팅(존 방식 구현) - 모니터링 - DB 구조의 전체 서버 (15000명 연결 유지 테스트)</h3>
</br>
<p>개발기간 : 2025.05.27-07.10</p>
</br>
<p>Language : C++</p>
<p>Skill : Redis, MySql, IOCP, Multithread</p>
<a href="https://github.com/IlsIkYoon/ChatMultiServer" target="_blank">🔗 프로젝트 보러 가기</a>

</br>
</br>
</br>

<h2>2. 게임 컨텐츠 서버</h2>
<h3>동시 접속 7000명 수용의 MMO환경 서버. 생성, 섹터 이동, 공격, 사망 처리, 존 방식으로 구현</h3>
</br>
<p>개발기간 : 2024.11.28-2025.03.16</p>
</br>
<p>Language : C++</p>
<p>Skill : Select, Sector</p>
<a href="https://github.com/IlsIkYoon/TCP_Fighter_report" target="_blank">🔗 프로젝트 보러 가기</a>
</br>
</br>
<p>아래 이미지를 클릭하면 영상으로 이동합니다</p>
<a href="https://www.youtube.com/watch?v=W_Aarnm9jrs" target="_blank">
  <img src="img/TCPFIGHTER.jpg" alt="게임 서버 영상" width="600"
       alt="YouTube Video"
       style="width: 600px; border-radius: 10px;">
</a>

</br>
</br>
</br>

<h2>3. 길찾기 GUI </h2>
<h3>A*, JPS를 이용한 길찾기 구현 및 GUI 시각화</h3>
</br>
<p>개발기간 : 2024.11.16-2025.01.13</p>
</br>
<p>Language : C++</p>
<p>Skill : A*, JPS</p>
<a href="https://github.com/IlsIkYoon/FindingWayProgram_report" target="_blank">🔗 프로젝트 보러 가기</a>
</br>
</br>
<p>아래 이미지를 클릭하면 영상으로 이동합니다</p>
<a href="https://youtu.be/ulu_kpi2pOo?si=eP62mDaPUl-3iX7t" target="_blank">
  <img src="img/findingAlgorithm.jpg" alt="길찾기 영상" width="600"
       alt="YouTube Video"
       style="width: 600px; border-radius: 10px;">
</a>

</br>
</br>
</br>
<h2>4. Lock-Free Queue</h2>
<h3>네트워크 라이브러리에서 사용하기 위한 락프리 큐 직접 구현</h3>

</br>
<p>개발기간 : 2025.03.24-28</p>
</br>

<p>Language : C++</p>
<p>Skill : MultiThread, Debugging</p>
<a href="https://github.com/IlsIkYoon/LockFreeQ" target="_blank">🔗 프로젝트 보러 가기</a>
</br>
</br>
</br>

<h2>5. 모니터링 클라이언트</h2>
<h3>모니터링 데이터를 실시간으로 확인하기 위한 클라이언트 구현</h3>

</br>
<p>개발기간 : 2025.07.22</p>
</br>
<p>Language : C#</p>
<p>Skill : .NET Winform</p>
<a href="https://github.com/IlsIkYoon/Winform-practice" target="_blank">🔗 프로젝트 보러 가기</a>

</br>
</br>
</br>
<h2>📞 Contact<h2></h2>
  <p>이메일 : ilsik95@naver.com</p>
  <p>블로그 : https://blog.naver.com/ilsik95</p>

