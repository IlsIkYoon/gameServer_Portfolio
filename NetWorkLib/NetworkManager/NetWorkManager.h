...
private:
	...
	//--------------------------------------------
	// 워커 쓰레드 생성 후 
	// 블로킹 _listensocket을 이용해 accept만을 실행
...
public:
	...
	void Regist_NetworkProtocolCode(unsigned char protocol);
	void Regist_PacketStickyKey(unsigned char stickyKey);
	//--------------------------------------------
	// 컨텐츠에서 전달하는 work
	//--------------------------------------------
	void RegistWork(CWork* threadWork);

	bool ConnectServer(std::wstring ip, unsigned short portNum, ULONG64* outSessionID);
};

enum class en_SendStatus
{
	Idle = 0,
	InProgress = 1
};